#!/Users/ravindrareddy/.pyenv/versions/3.12.12/bin/python3       ## change this with your python env
...
def main():
    ...
    try:
        ...
        # Exclude the totals row which contains <th> tags mixed in
        rows = driver.find_elements(
            By.XPATH,
            "//div[@id='attendanceDiv']//table/tbody/tr[td and not(th)]"
        )
        ...
        for row in rows:
            ...
            if len(cols) >= 4:
                ...
                # Page shows plain float like "100.00", no % sign
                try:
                    p_val = float(percent_str)
                except:
                    p_val = 0.0
                ...
                subjects.append({
                    ...
                    "full_name": name,
                    "count": count_str,
                    "percent_str": f"{p_val:.2f}%",  # Add % for display
                    "p_val": p_val
                })
